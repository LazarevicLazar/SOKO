...
FALCON_DIR="$ROOT_DIR/../online-offline-sigs-(precomputed Falcon)/falcon-lazy"
TARGET="bench_multisigner_refill"
...
echo "===== Building Consume-Only Refill Simulator ====="
echo
...
if NVCC=$(detect_nvcc); then
    echo "CUDA toolchain detected ($NVCC). Preparing GPU helper DLL..."
    if CL_DIR=$(detect_cl_dir); then
        ...
        if [ "$GPU_REQUIRED" -eq 1 ]; then
            echo "MSVC host compiler not found and --gpu-required was set." >&2
            exit 1
        fi
        echo "cl.exe not detected. Building CPU-only simulator (SOKO will be unavailable)."
    fi
else
    if [ "$GPU_REQUIRED" -eq 1 ]; then
        echo "nvcc not found and --gpu-required was set." >&2
        exit 1
    fi
    echo "nvcc not found. Building CPU-only simulator (SOKO will be unavailable)."
fi

echo
echo "Compiling simulator source..."
"$CC" $CFLAGS $GPU_DEFS -I"$INCLUDE_DIR" -I"$ROOT_DIR" -I"$FALCON_DIR" \
    -c "$BENCH_DIR/bench_multisigner_refill.c" -o "$BUILD_DIR/bench_multisigner_refill.o"
...
"$CC" $CFLAGS -I"$FALCON_DIR" -c "$FALCON_DIR/codec.c" -o "$BUILD_DIR/msim_fal_codec.o"
"$CC" $CFLAGS -I"$FALCON_DIR" -c "$FALCON_DIR/common.c" -o "$BUILD_DIR/msim_fal_common.o"
"$CC" $CFLAGS -I"$FALCON_DIR" -c "$FALCON_DIR/falcon.c" -o "$BUILD_DIR/msim_fal_falcon.o"
"$CC" $CFLAGS -I"$FALCON_DIR" -c "$FALCON_DIR/fft.c" -o "$BUILD_DIR/msim_fal_fft.o"
"$CC" $CFLAGS -I"$FALCON_DIR" -c "$FALCON_DIR/fpr.c" -o "$BUILD_DIR/msim_fal_fpr.o"
"$CC" $CFLAGS -I"$FALCON_DIR" -c "$FALCON_DIR/keygen.c" -o "$BUILD_DIR/msim_fal_keygen.o"
"$CC" $CFLAGS -I"$FALCON_DIR" -c "$FALCON_DIR/rng.c" -o "$BUILD_DIR/msim_fal_rng.o"
"$CC" $CFLAGS -I"$FALCON_DIR" -c "$FALCON_DIR/shake.c" -o "$BUILD_DIR/msim_fal_shake.o"
"$CC" $CFLAGS -I"$FALCON_DIR" -c "$FALCON_DIR/sign.c" -o "$BUILD_DIR/msim_fal_sign.o"
"$CC" $CFLAGS -I"$FALCON_DIR" -c "$FALCON_DIR/vrfy.c" -o "$BUILD_DIR/msim_fal_vrfy.o"

FALCON_OBJS="$BUILD_DIR/msim_fal_codec.o $BUILD_DIR/msim_fal_common.o $BUILD_DIR/msim_fal_falcon.o $BUILD_DIR/msim_fal_fft.o $BUILD_DIR/msim_fal_fpr.o $BUILD_DIR/msim_fal_keygen.o $BUILD_DIR/msim_fal_rng.o $BUILD_DIR/msim_fal_shake.o $BUILD_DIR/msim_fal_sign.o $BUILD_DIR/msim_fal_vrfy.o"

echo
echo "Linking $TARGET..."
"$CC" $CFLAGS $LDFLAGS -o "$BUILD_DIR/$TARGET.exe" "$BUILD_DIR/bench_multisigner_refill.o" \
    "$BUILD_DIR/soko_falcon_common.o" "$BUILD_DIR/soko_cpu_backend.o" \
    "$BUILD_DIR/soko_cpumt_backend.o" "$BUILD_DIR/soko_gpu_backend.o" \
    $FALCON_OBJS $LIBS
