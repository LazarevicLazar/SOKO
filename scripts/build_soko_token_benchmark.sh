...
# Dedicated build script for the SOKO token benchmark artifact.
...
for arg in "$@"; do
    case "$arg" in
        --gpu-required)
            ...
            ;;
        --check-gpu-env)
            echo "===== SOKO GPU Environment Check ====="
...
"$CC" $CFLAGS $GPU_DEFS -I"$INCLUDE_DIR" -I"$FALCON_DIR" -I"$ROOT_DIR" \
    -c "$BENCH_DIR/soko_token_benchmark.c" -o "$BUILD_DIR/soko_token_benchmark.o"
...
"$CC" $CFLAGS $LDFLAGS -o "$BUILD_DIR/$TARGET.exe" \
    "$BUILD_DIR/soko_token_benchmark.o" "$BUILD_DIR/soko_falcon_common.o" \
